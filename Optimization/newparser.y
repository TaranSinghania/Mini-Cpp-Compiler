...
void codegen()
{
    ...
    sprintf(res,"%s = %s %s %s",temp,st[top-3],st[top-2],st[top-1]);
    fprintf(icgop, "%s\n",res);
    free(res);
...
void codgen_un()
{
    ...
    if((!strcmp(st[top - 2],"++")) || (!strcmp(st[top - 2],"--"))){         //pre increment
    printf("%s = %s %c %d\n", temp, st[top-1], st[top-2][0], 1);

    char *res = (char *)malloc(sizeof(char)*50);
    sprintf(res,"%s = %s %c %d", temp, st[top-1], st[top-2][0], 1);
    fprintf(icgop, "%s\n",res);
    free(res);
    ...
    char *res1 = (char *)malloc(sizeof(char)*50);
    sprintf(res1,"%s = %s", st[top - 1], temp);
    fprintf(icgop, "%s\n",res1);
    free(res1);


    }
    else if((!strcmp(st[top - 1],"++")) || (!strcmp(st[top - 1],"--"))){       //post increment
    printf(" %s = %s %c %d\n", temp, st[top-2], st[top-1][0], 1);

    char *res2 = (char *)malloc(sizeof(char)*50);
    sprintf(res2,"%s = %s %c %d", temp, st[top-2], st[top-1][0], 1);
    fprintf(icgop, "%s\n",res2);
    free(res2);
    ...
    char *res3 = (char *)malloc(sizeof(char)*50);
    sprintf(res3,"%s = %s", st[top - 2], temp);
    fprintf(icgop, "%s\n",res3);
    free(res3);
...
void codegen_syns(){
  ...
  char *res = (char *)malloc(sizeof(char)*50);
  sprintf(res,"%s %c %s %c %s", st[top-3], st[top-2][1], st[top-3], st[top-2][0], st[top - 1]);
  fprintf(icgop, "%s\n",res);
  free(res);
...
void codegen_assign(){
  ...
  char *res = (char *)malloc(sizeof(char)*50);
  sprintf(res,"%s = %s", st[top-2], st[top-1]);
  fprintf(icgop, "%s\n",res);
  free(res);
...
void for2() //ICG for condition check
{
    ...
    printf("%s = not %s %s %s\n",temp,st[top-3],st[top-2],st[top-1]);
    char *res = (char *)malloc(sizeof(char)*50);
    sprintf(res,"%s = not %s %s %s",temp,st[top-3],st[top-2],st[top-1]);
    fprintf(icgop, "%s\n",res);
    free(res);
    ...
    char *res1 = (char *)malloc(sizeof(char)*50);
    sprintf(res1,"if %s goto L%d",temp,label[ltop - 1]);
    fprintf(icgop, "%s\n",res1);
    free(res1);
	...
	printf("goto L%d\n", label[ltop - 3]);
    char *res2 = (char *)malloc(sizeof(char)*50);
    sprintf(res2,"goto L%d", label[ltop - 3]);
    fprintf(icgop, "%s\n",res2);
    free(res2);

  printf("L%d :\n", label[ltop - 2]);
  char *res3 = (char *)malloc(sizeof(char)*50);
  sprintf(res3,"L%d :", label[ltop - 2]);
  fprintf(icgop, "%s\n",res3);
  free(res3);
...
void for3() //incrementing and executing loop again
{
  printf("goto L%d\n", label[ltop - 4]);
  char *res = (char *)malloc(sizeof(char)*50);
  sprintf(res,"goto L%d", label[ltop - 4]);
  fprintf(icgop, "%s\n",res);
  free(res);

  printf("L%d :\n", label[ltop - 3]);
  char *res1 = (char *)malloc(sizeof(char)*50);
  sprintf(res1,"L%d :", label[ltop - 3]);
  fprintf(icgop, "%s\n",res1);
  free(res1);
}
...
void for4() //go to autoincrement and print label after loop
{
	printf("goto L%d \n",label[ltop - 2]);
    char *res = (char *)malloc(sizeof(char)*50);
    sprintf(res,"goto L%d ",label[ltop - 2]);
    fprintf(icgop, "%s\n",res);
    free(res);

	printf("L%d : \n",label[ltop - 1]);
    char *res1 = (char *)malloc(sizeof(char)*50);
    sprintf(res1,"L%d : ",label[ltop - 1]);
    fprintf(icgop, "%s\n",res1);
    free(res1);
...
void if1()
{
 ...
 printf("if %s goto L%d\n",temp,lnum);
 char *res1 = (char *)malloc(sizeof(char)*50);
 sprintf(res1,"if %s goto L%d",temp,lnum);
 fprintf(icgop, "%s\n",res1);
 free(res1);
