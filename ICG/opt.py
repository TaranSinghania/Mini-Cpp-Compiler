...
def findLoopLines(lines):
    ...
    for i in range(len(lines)-1):
        ...
        if(len(lines[i].split())==4 and lines[i].split()[0] == 'if' and lines[i+1].split()[0]=='goto'):
            startlabel = lines[i-1].split()[0]
            labelindex[startlabel] = i-1
            endlabel = lines[i].split()[-1]
        if(len(lines[i].split()) == 2 and lines[i].split()[1] == ':' and lines[i].split()[0]==endlabel):
            gotoindex[startlabel] = i-1
    ...
    for i in lines:
        ...
        if k in blocks:
            if(len(i.split()) == 3):
                ...
                lhs, ass, op1, arg1, op2, arg2 = i.split()
                print(op1+op2, arg1, arg2, lhs)
...
for i in range(len(lines)-1):
    ...
    if("not" in lines[i].split()):
        ...
        if(op=="<"):
            ...
            op="=="
        tokens1=lines[i+1].split()
        lines[i+1]=tokens1[0]+" "+tokens[3]+op+tokens[5]+" "+tokens1[2]+" "+tokens1[3]
        topop.append(i-count)
        count+=1
